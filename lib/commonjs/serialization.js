...
function deserializeResult(json) {
  ...
  throw new _types.KoraError(_types.KoraErrorCode.INVALID_RESPONSE, `Unknown result type: ${String(raw.type)}`);
}
...
function deserializeError(code, message) {
  const errorCode = isKoraErrorCode(code) ? code : _types.KoraErrorCode.UNKNOWN;
  return new _types.KoraError(errorCode, message);
}
function isKoraErrorCode(code) {
  return Object.values(_types.KoraErrorCode).includes(code);
}
